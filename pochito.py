...
class Pochito(pygame.sprite.Sprite):
	...
	def __init__(self, screen, x, y):
		...
		self.move_left_attak = [
			pygame.image.load('src/pochito_attak_left1.png'),
			pygame.image.load('src/pochito_attak_left2.png')
		]
		self.move_right = [
	...
	def drawing(self):
		if self.health >= 1:
			...
			if self.direction == "left_attak":
				self.screen.blit( self.move_left_attak[self.image_anim_count], ( self.x, self.y, ) )
				...
				if self.rect.top > self.screen_rect.top + 200:
					if self.rect.right < self.screen_rect.right:
						if self.rect.left > self.screen_rect.left:
							...
							if self.direction == "left_attak":
								if self.image_anim_count < len(self.move_left_attak) - 1:
